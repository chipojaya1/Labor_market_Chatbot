...
class LaborMarketDatabase:
    ...
    def _load_tables(self) -> None:  # type: ignore[override]
        ...
        for table_name, filename in self.table_files.items():
            ...
            frame = frame.loc[:, ~frame.columns.astype(str).str.match(r"^Unnamed", na=False)]

            if table_name == "bls_macro_indicators" and "year" in frame.columns:
                frame = frame[frame["year"] == 2024]
